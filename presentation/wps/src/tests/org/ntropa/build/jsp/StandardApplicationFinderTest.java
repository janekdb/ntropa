...
public class StandardApplicationFinderTest extends TestCase {
    ...
    protected void setUp() throws Exception, IOException {
        ...
        final String TOP_FOLDER = new NtropaTestWorkDirHelper().getWorkDir()
                + "/tests.org.ntropa.build.jsp.StandardApplicationFinderTest";
    ...
    /**
     * When thousands of files were added ntropa was failing with "Too many open
     * files". This was because the InputStream used to load properties with was
     * not being closed.
     */
    public void testFilesAreClosed() throws FileLocationException {
        ...
        /*
         * 2100 was not enough on Ubuntu 8.04.1 to expose the problem. 4100 was
         * sufficient.
         */
        for (int i = 1; i < 4100; i++) {
            assertEquals("Fault with run #" + i, "org.ntropa.runtime.sao.ResultList", saf.getClazzName(saf
                    .getSaoData("result-list")));
        }
    }
    ...
    public void testOverrideOfApplicationProperties() throws FileLocationException {
        ...
        StandardApplicationFinder saf = new StandardApplicationFinder(new File(_fixtureRoot, "test-4"),
                new FileLocation("anywhere.html"));
        assertEquals("Fixture for test-4 was okay", "name-1-from-application-properties", saf.getClazzName(saf
                .getSaoData("name-1")));

        /* name-common is defined in options.properties */
        assertEquals("name-common data was taken from options.properties", "common-from-options-properties", saf
                .getClazzName(saf.getSaoData("common")));
    ...
    public void testOptionalParamFileCheckedInAlphanumericOrder() throws FileLocationException {
        StandardApplicationFinder saf = new StandardApplicationFinder(new File(_fixtureRoot, "test-5"),
                new FileLocation("anywhere.html"));
        assertEquals("name-1 was okay", "name-1-from-abc", saf.getClazzName(saf.getSaoData("name-1")));
        assertEquals("name-2 was okay", "name-2-from-abc", saf.getClazzName(saf.getSaoData("name-2")));
        assertEquals("name-3 was okay", "name-3-from-def", saf.getClazzName(saf.getSaoData("name-3")));
        assertEquals("name-4 was okay", "name-4-from-xyz", saf.getClazzName(saf.getSaoData("name-4")));
